...
export const updateStatus = async (req, res, next) => {
  try {
    ...
    // Kiểm tra nếu trạng thái chuyển thành "waiting"
    if (req.body.status === "waiting") {
      // Lấy thông tin khách hàng từ đơn hàng
      const { user_id, id_order } = order;
      ...
      const mailOptions = {
        from: EMAIL_USERNAME,
        to: user.email,
        subject: "Sneaker Store",
        html: `
      <div style="font-family: Arial, sans-serif;">
        <h2 style="color: #333;">Đơn hàng của bạn đã được xác nhận</h2>
        <p style="color: #333;">
          Đơn hàng của bạn (Mã đơn: ${order._id}) đang trên đường vận chuyển tới bạn.
          Vui lòng đăng nhập vào website Sneaker Store để xem chi tiết.
        </p>
      </div>
    `,
      };
...
export const createOrder = async (req, res) => {
  try {
    ...
    for (const product of newOrder.products) {
      ...
      product.total_price = existingProduct.price;
    }
    ...
    const transporter = nodemailer.createTransport({
      service: "gmail",
      auth: {
        user: EMAIL_USERNAME, // Thay bằng tài khoản Gmail của bạn
        pass: EMAIL_PASSWORD, // Thay bằng mật khẩu Gmail của bạn
      },
    });
    const statusForCustomer = "Chờ xác nhận";
    const orderAddress = JSON.parse(newOrder.address);
  ...
  <html>
    ...
    <body>
      <div class="container">
        ...
        <p class="order-info">Mã đơn hàng: ${newOrder._id}</p>
        <p class="order-info">Tên người đặt: ${orderAddress.name}</p>
        <p class="order-info">Số điện thoại: ${orderAddress.phone}</p>
        <p class="order-info">Địa chỉ: ${orderAddress.address}</p>
        <p class="order-info">Mã người dùng: ${newOrder.user_id}</p>
        <p class="order-info">Trạng thái: ${statusForCustomer}</p>
        <p class="order-info">Kiểu thanh toán: ${newOrder.payment_type}</p>
        <p class="order-info">Tổng tiền: ${newOrder.total_price}</p>
